...
Earth.prototype.getRadius = function() {
	...
	for ( var i=1; i <= tests; i++ ) {
		...
		this.raycaster.setFromCamera( Earth.normalizeRaycast( new THREE.Vector2( 0.5, test_y ) ), this.camera );
		
		test_step *= 0.75;
		
		if ( this.raycaster.intersectObjects( [this.sphere] ).length ) {
			test_y -= test_step;
			test_y = Math.max( 0.005, test_y );
			intersects++;
		} else {
			test_y += test_step;
			test_y = Math.min( 0.5, test_y );			
		}
	...
	if ( intersects == 0 ) {
		return 0;
	} else if ( intersects == tests ) {
		return this.elementSize.y / 2;
	} else {	
		return this.elementSize.y / 2 - this.elementSize.y * test_y;
	}
...
Earth.Occludable.prototype.updatePositions = function() {
	...
	this.worldPosition = Earth.latLngToWorld( this.location, Earth.earthRadius + this.offset );
	this.elementPosition = Earth.worldToElement( this.worldPosition, this.earth.elementSize, this.earth.camera );
	this.distance = this.earth.camera.position.distanceTo( this.worldPosition );			
...
Earth.Occludable.prototype.updateOcclusion = function() {
	...
	if ( this.distance < this.earth.camera.position.length() ) { // between earth and camera	
		...
		this.earth.raycaster.setFromCamera( Earth.normalizeRaycast( Earth.normalizeElement( this.elementPosition, this.earth.elementSize) ), this.earth.camera );
		var intersection = this.earth.raycaster.intersectObjects( [this.earth.sphere] );
		
		var occluded = intersection.length != 0;
